...
export default class DxfArrayScanner {
    ...
    rewind(numberOfGroups: number = 1): void {
 ...
 */
function parseGroupValue(code: number, value: string): any {
    if (code <= 9) return value;
    if (code >= 10 && code <= 59) return parseFloat(value.trim());
    if (code >= 60 && code <= 99) return parseInt(value.trim(), 10);
    if (code >= 100 && code <= 109) return value;
    if (code >= 110 && code <= 149) return parseFloat(value.trim());
    if (code >= 160 && code <= 179) return parseInt(value.trim(), 10);
    if (code >= 210 && code <= 239) return parseFloat(value.trim());
    if (code >= 270 && code <= 289) return parseInt(value.trim(), 10);
    if (code >= 290 && code <= 299) return parseBoolean(value.trim());
    if (code >= 300 && code <= 369) return value;
    if (code >= 370 && code <= 389) return parseInt(value.trim(), 10);
    if (code >= 390 && code <= 399) return value;
    if (code >= 400 && code <= 409) return parseInt(value.trim(), 10);
    if (code >= 410 && code <= 419) return value;
    if (code >= 420 && code <= 429) return parseInt(value.trim(), 10);
    if (code >= 430 && code <= 439) return value;
    if (code >= 440 && code <= 459) return parseInt(value.trim(), 10);
    ...
    if (code >= 1060 && code <= 1071) return parseInt(value.trim(), 10);
...
function parseBoolean(str: string): boolean {
    ...
    throw new TypeError('String \'' + str + '\' cannot be cast to Boolean type');
}
