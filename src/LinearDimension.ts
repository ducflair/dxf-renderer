...
export class DimensionLayout {
    ...
    AddLine(start: Vector2, end: Vector2, color: number | null = null): void {
        this.lines.push({ start, end, color });
    }

    /** Add one or more triangles. */
    AddTriangles(vertices: Vector2[], indices: number[], color: number | null = null): void {
        this.triangles.push({ vertices, indices, color });
    }

    AddText(text: string, size: number, angle: number, color: number, position: Vector2): void {
...
export type LinearDimensionParams = {
...
export class LinearDimension {
    ...
    styleResolver: (paramName: string) => any;
    textWidthCalculator: (text: string, fontSize: number) => number;
    isValid: boolean;
    vBase!: Vector2;
    vDim!: Vector2;
    vDimNorm!: Vector2;
    ...
    /**
     * @param params
     * @param styleResolver Provides value for a requested style parameter.
     * @param textWidthCalculator Get text width in model space units for a given text and font size (height).
     */
    constructor(
        params: LinearDimensionParams,
        styleResolver: (paramName: string) => any,
    ...
    GenerateLayout(): DimensionLayout {
        ...
        const DrawExtLine = (basePt: Vector2, dimPt: Vector2) => {
            ...
            vExt.normalize();
            const startPt = basePt.clone();
            if (extOffset !== 0) {
                startPt.add(vExt.clone().multiplyScalar(extOffset));
            }
            const endPt = dimPt.clone();
            if (extExt !== 0) {
                endPt.add(vExt.clone().multiplyScalar(extExt));
            }
            result.AddLine(startPt, endPt, extColor);
        };
        ...
        for (let i = 0; i < 2; i++) {
            ...
            const rotAngle = -this.vDim.angle();
    ...
    _CreateArrowShape(layout: DimensionLayout, transform: Matrix3, color: number | null): void {
    ...
    _CreateTick(layout: DimensionLayout, transform: Matrix3, color: number | null): void {
        layout.AddLine(
    ...
    _GetText(): string {
        ...
        if (typeof decSep === "number" && !isNaN(decSep)) {
            decSep = String.fromCharCode(decSep);
        }
        if (decSep !== ".") {
            measText = measText.replace(".", String(decSep));
        }
