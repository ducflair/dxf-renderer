...
export class DxfScene {
    ...
    dimStyles: Map<string, any>;
    vars: Map<string, any>;
    fontStyles: Map<string, any>;
    ...
    async Build(dxf, fontFetchers) {
        ...
        if(dxf.tables && dxf.tables.layer) {
            for (const [, layer] of Object.entries(dxf.tables.layer.layers as any) as [string, any][]) {
        ...
        if(dxf.tables && dxf.tables.dimstyle) {
            for (const [, style] of Object.entries(dxf.tables.dimstyle.dimStyles as any) as [string, any][]) {
                this.dimStyles.set(style.name, style)
            }
        }

        if (dxf.tables && dxf.tables.style) {
            for (const [, style] of Object.entries(dxf.tables.style.styles as any) as [string, any][]) {
    ...
    *_DecomposeDimension(entity, blockCtx) {
     ...
     *  `earcut` library.
     */
    _TransformBoundaryLoop(loop: any, transform: any, result: any[] = []) {
        if (!result) {
            result = []
        }
        for (const v of loop) {
    ...
    *_DecomposeSpline(entity, blockCtx = null) {
    ...
    _GetEntityColor(entity, blockCtx = null) {
        let color: number = ColorCode.BY_LAYER
