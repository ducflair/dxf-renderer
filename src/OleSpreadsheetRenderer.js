...
function ExtractPackage(oleData) {
...
function DrawBorder(ctx, side, x1, y1, x2, y2) {
    if (!side?.style) {
        return
    }
    ctx.save()
...
function WrapText(ctx, text, maxWidth, wrapText) {
...
function DrawText(ctx, cell, x, y, width, height) {
...
function DecodeRange(range) {
    ...
    const decodeColumn = label => [...label].reduce((value, char) => value * 26 + char.charCodeAt(0) - 64, 0)
    return {
...
function GetRangeBorder(worksheet, range, fallback) {
    return {
...
export async function ReadOleSpreadsheet(oleData) {
...
export async function RenderOleSpreadsheet(oleData) {
    ...
    const rowOffsets = [0]
    for (const width of columnWidths) columnOffsets.push(columnOffsets.at(-1) + width)
    for (const height of rowHeights) rowOffsets.push(rowOffsets.at(-1) + height)

    const logicalWidth = columnOffsets.at(-1)
    const logicalHeight = rowOffsets.at(-1)
